...
class EvaluationModel extends BaseModel{
    ...
    public function getRecommendationsforeachSemesterFacultySide($id){
        $stmt = $this->db->prepare("SELECT * FROM `faculty_evaluation_summary` WHERE `id` = ? ");
        $stmt->execute([$id]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC); 
    }
    ...
    public function searchCertificate($keyword){
        ...
        (
            `seminar_title` LIKE :keyword OR
            `seminar_name` LIKE :keyword OR
            `faculty_name` LIKE :keyword OR
            `uploaded_at` LIKE :keyword 
        )
        ORDER BY `id` DESC ";
