...
class Helpers extends BaseModel{
    ...
    public function RecoveryAccount($studentID)
    {
        $mail = new PHPMailer(true);
        ...
        if($stmt->rowCount() > 0){
            ...
            $emailTo = $row['student_email'];
            $Student_fullname = $row['student_name'];
        }else{
            return false;
        }
        ...
        try {
            ...
            $mail->addAddress($emailTo);
            $mail->isHTML(true);
            $mail->Subject = 'FEAPP Account Recovery';
            ...
            <div style="max-width: 500px; margin: auto; background-color: #ffffff; border-radius: 12px; padding: 30px; box-shadow: 0 0 10px rgba(50, 205, 50, 0.3); border: 2px solid rgba(50, 205, 50, 0.4); position: relative;">
                <div style="text-align: center;">
                <img src="cid:' . $cid . '" alt="Logo" style="height: 80px; margin-bottom: 20px;">
                <h2 style="color: #0F828C; margin: 10px 0;">Account Recovery</h2>
                </div>
                
                <p style="text-align: center; font-size: 15px; color: #333;">Hi <strong>' . htmlspecialchars($Student_fullname) . '</strong>,</p>
                <div style="text-align: center; margin: 30px 0;">
                    <p style="text-align: center; font-size: 14px; color: #444;">Registered Email: '. htmlspecialchars ($emailTo).'</p>
                    <p style="text-align: center; font-size: 14px; color: #444;">Student no. as password: '. htmlspecialchars ($studentID).'</p>
                </div>
