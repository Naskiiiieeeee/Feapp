...
if (isset($_GET['token'])) {
    ...
    if ($code === $salt) {
        $facultyData = $vm->getIndiEvaluationResult($code, $email);
    } else {
        echo '<script>alert("Invalid token!"); window.location="ViewEvaluationResults.php";</script>';
        exit;
    }
}
    ...
    <div class="col-xl-4">
      <div class="card">
        ...
        <?php if ($firstRow): ?>
            ...
            <h2 id=""><?= htmlspecialchars($firstRow['faculty_name']) ?></h2>
            <h3 id=""><?= htmlspecialchars($firstRow['faculty_dep']) ?></h3>
        <?php else: ?>
